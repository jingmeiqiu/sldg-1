        ...
        do kk1=1,ny
            do lx=1,6
                do ly=1,6
                    den=abs( exact(x(kk0)+xg(lx)*dx ,y(kk1)+xg(ly)*dy,time_final)  &
                        - polynomial(element(kk0,kk1)%umodal(1:n_moment), x(kk0)+xg(lx)*dx,x(kk0),dx,y(kk1)+xg(ly)*dy,y(kk1),dy,n_moment ) )
            ...
            do lx=1,6
                do ly=1,6
                    xrg =  x(kk0)+xg(lx)*dx
                    yrg =  y(kk1)+xg(ly)*dy
                    error3=max(error3,abs( exact(xrg ,yrg,time_final) - polynomial( element(kk0,kk1)%umodal(1:n_moment),xrg,x(kk0),dx,yrg,y(kk1),dy,n_moment )  ))
                enddo
            enddo
